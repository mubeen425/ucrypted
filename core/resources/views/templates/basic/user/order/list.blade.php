                ...
                <form class="d-flex gap-2 flex-wrap">
                    <div class="flex-fill">
                        <div class="input-group position-relative">
                            <select name="order_type" class="form-control form--control submit-form-on-change form-select select2"
                    ...
                    <div class="flex-fill">
                        <div class="position-relative">
                            <select class="form-control form--control submit-form-on-change form-select select2" name="order_side"
                                data-minimum-results-for-search="-1" data-width="150px">
                        ...
                        @forelse($orders as $order)
                            <tr class="tr-{{ $order->id }}">
                                ...
                                <td>
                                    <div class="order--amount-rate-wrapper">
                                        <span class="order--amount d-block">
                                            <span class="order--amount-value">{{ showAmount($order->amount, currencyFormat: false) }}</span>
                                            @if ($order->status == Status::ORDER_OPEN)
                                                <span class="amount-rate-update" data-order='@json($order->only('amount', 'rate', 'id'))' data-update-filed="amount">
                                                    <i class="las la-edit"></i>
                                                </span>
                                            @endif
                                        </span>
                                        <span class="order--rate d-block">
                                            <span class="order--rate-value">{{ showAmount($order->rate, currencyFormat: false) }}</span>
                                            @if ($order->status == Status::ORDER_OPEN)
                                                <span class="amount-rate-update" data-order='@json($order->only('amount', 'rate', 'id'))' data-update-filed="rate">
                                                    <i class="las la-edit"></i>
                                                </span>
                                            @endif
                                        </span>
                                ...
                                <td>
                                    <div>
                                        <span class="d-block">
                                            {{ showAmount($order->charge, currencyFormat: false) }} {{ __($marketCurrencySymbol) }}
                                        </span>
                                        <span>
                                            {{ showAmount($order->total, currencyFormat: false) }} {{ __($marketCurrencySymbol) }}
                                        </span>
                                    </div>
                                </td>
                                <td>
                                    <div class="progress-wrapper">
                                        <div class="progress">
                                            <div class="progress-bar" role="progressbar" style="width: {{ getAmount($order->filed_percentage) }}%;"
                                ...
                                @if ($actionShow)
                                    <td>
                                        @if ($order->status == Status::ORDER_OPEN)
                                            ...
                                            @endphp
                                            <div>
                                                <button type="button" class="btn btn--danger btn--cancel outline btn--sm confirmationBtn ms-2"
                                                    data-question="{{ $cancelMessage }}" data-action="{{ route('user.order.cancel', $order->id) }}">
